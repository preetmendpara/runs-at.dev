...
test('points the canonical at the name, not the registry apex', () => {
  // The root layout sets metadataBase to https://runs-at.dev, so a relative
  ...
  assert.equal(m.alternates.canonical, 'https://shrey.runs-at.dev');
  assert.equal(m.openGraph.url, 'https://shrey.runs-at.dev');
});

test('bypasses the title template so the suffix is not doubled', () => {
  // Layout template is '%s — runs-at.dev'; a plain string title would render
  // 'shrey.runs-at.dev — runs-at.dev'.
  const m = cardMetadata({ name: 'shrey', record, profile });
  assert.equal(typeof m.title, 'object');
  assert.ok(m.title.absolute.includes('shrey.runs-at.dev'));
  assert.ok(!m.title.absolute.includes('— runs-at.dev'));
});

test('carries the owner name into the title when GitHub has one', () => {
  const m = cardMetadata({ name: 'shrey', record, profile });
  assert.equal(m.title.absolute, 'Shrey (shrey.runs-at.dev)');
});
...
test('falls back to the hostname alone when GitHub has no display name', () => {
  ...
  assert.equal(m.title.absolute, 'shrey.runs-at.dev');
});
...
test('describes the name when there is no bio', () => {
  for (const p of [null, {}, { bio: '   ' }, { bio: null }]) {
    ...
    assert.ok(m.description.includes('shrey.runs-at.dev'), JSON.stringify(p));
...
test('overrides the twitter tags rather than inheriting the registry ones', () => {
  ...
  // claimed name unfurl as 'runs-at.dev — free subdomains'.
  const m = cardMetadata({ name: 'shrey', record, profile });
...
test('marks the card as a profile, not a website', () => {
  ...
  assert.equal(m.openGraph.siteName, 'runs-at.dev');
});
...
test('survives a record with no profile fetched at all', () => {
  ...
  assert.equal(m.title.absolute, 'lucas.runs-at.dev');
  assert.equal(m.alternates.canonical, 'https://lucas.runs-at.dev');
});
