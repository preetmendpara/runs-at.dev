// The translation between what the editor form shows (text in boxes) and the
// `records` object the registry stores. Kept out of the component so the
// parsing that decides what actually gets committed is testable on its own,
// rather than only reachable by rendering a form.

// The four shapes a record can legally take, which is the DNS coexistence
// rule in schema.js turned into something you can pick from: CNAME must stand
// alone, URL must stand alone, A/TXT/MX may be combined, and no records at
// all means the name keeps serving its profile card.
export const MODES = ['card', 'cname', 'advanced', 'url'];

export function modeOf(records = {}) {
  if ('URL' in records) return 'url';
  if ('CNAME' in records) return 'cname';
  if ('A' in records || 'AAAA' in records || 'TXT' in records || 'MX' in records) return 'advanced';
  return 'card';
}

export function linesToList(text) {
  return String(text ?? '').split('\n').map((l) => l.trim()).filter(Boolean);
}

// "10 mx.example.com" per line — the priority and the host, in the order they
// appear in every DNS console, rather than two inputs per row. A malformed
// line still produces an entry (with a NaN priority or an empty value) rather
// than being dropped, so schema validation reports it instead of the form
// quietly discarding what someone typed.
export function parseMx(text) {
  return linesToList(text).map((line) => {
    const [priority, ...rest] = line.split(/\s+/);
    // The host half is validated against the same HOSTNAME pattern as CNAME,
    // so it gets the same normalisation -- an MX pasted as
    // "10 mx.example.com." is the ordinary way a zone file writes it.
    return { priority: Number(priority), value: normalizeHostname(rest.join(' ')) };
  });
}

export function mxToLines(mx = []) {
  return (Array.isArray(mx) ? mx : [])
    .map((e) => `${e?.priority ?? ''} ${e?.value ?? ''}`.trim())
    .join('\n');
}

// Only the active mode's fields are read. Switching mode therefore drops the
// records that could not have coexisted with the new shape anyway, which is
// the conservative reading: the record you can see on screen is exactly the
// record that gets written, with nothing carried invisibly underneath it.
// DNS consoles hand out hostnames in forms the schema's HOSTNAME pattern
// rejects, and the resulting "CNAME must be a hostname" is a maddening thing
// to read about a value that plainly is one:
//
//   - a trailing dot is the fully-qualified form (`example.com.`), valid
//     notation that several providers display and copy verbatim
//   - DNS is case-insensitive, so `Example.COM` names the same host, but the
//     pattern only matches lowercase
//
// Normalising beats rejecting: the stored record stays canonical (which is
// what sync-dns and the schema both want) and the person pasting from their
// provider is not asked to notice a full stop. Deliberately NOT applied to
// TXT values, which are opaque and case-sensitive -- lowercasing a
// verification token would break it.
export function normalizeHostname(value) {
  return String(value ?? '').trim().replace(/\.+$/, '').toLowerCase();
}

export function buildRecords(mode, fields = {}) {
  if (mode === 'cname') {
    const value = normalizeHostname(fields.cname);
    return value ? { CNAME: value } : {};
  }
  if (mode === 'url') {
    const value = String(fields.url ?? '').trim();
    return value ? { URL: value } : {};
  }
  if (mode === 'advanced') {
    const out = {};
    const a = linesToList(fields.a);
    const aaaa = linesToList(fields.aaaa);
    const txt = linesToList(fields.txt);
    const mx = parseMx(fields.mx);
    if (a.length) out.A = a;
    if (aaaa.length) out.AAAA = aaaa;
    if (txt.length) out.TXT = txt;
    if (mx.length) out.MX = mx;
    return out;
  }
  return {};
}

// Subdomain record types, in the order the form offers them. No URL: the app
// only ever looks up the claimed name itself, so a redirect one label down
// could never be served, and schema.js rejects it.
export const SUBDOMAIN_TYPES = ['TXT', 'CNAME', 'A', 'AAAA', 'MX'];

// The form holds subdomains as flat {label, type, value} rows rather than as
// the nested object the record stores. One row is one thing a provider asked
// for ("put this TXT at _vercel"), which is how people actually receive them,
// and it keeps a label that legitimately carries two types -- A and TXT, say --
// from needing nested UI. Rows sharing a label merge back into one entry here,
// so the coexistence rules apply to the merged result exactly as they would to
// a hand-written file.
export function buildSubdomains(rows = []) {
  const out = {};
  for (const row of rows) {
    const label = String(row?.label ?? '').trim().toLowerCase();
    const type = row?.type;
    if (!label || !SUBDOMAIN_TYPES.includes(type)) continue;

    const value = parseValue(type, row?.value);
    // An empty row is someone who has added the row but not filled it in yet,
    // not a request to write an empty record.
    if (value === null) continue;

    out[label] ??= {};
    out[label][type] = value;
  }
  return out;
}

function parseValue(type, raw) {
  if (type === 'CNAME') {
    const v = normalizeHostname(raw);
    return v ? v : null;
  }
  if (type === 'MX') {
    const mx = parseMx(raw);
    return mx.length ? mx : null;
  }
  const list = linesToList(raw);
  return list.length ? list : null;
}

// The inverse, for seeding the form from a stored record.
export function subdomainsToRows(subdomains = {}) {
  const rows = [];
  for (const label of Object.keys(subdomains ?? {})) {
    const entry = subdomains[label] ?? {};
    for (const type of Object.keys(entry)) {
      rows.push({ label, type, value: formatValue(type, entry[type]) });
    }
  }
  return rows;
}

function formatValue(type, value) {
  if (type === 'MX') return mxToLines(value);
  if (Array.isArray(value)) return value.join('\n');
  return String(value ?? '');
}

// --- profile ---

// The form holds profile links as flat {label, url} rows, seeded by
// profileToRows and folded back by buildProfile. Same discipline as
// subdomains rows: a row that is entirely empty is someone who has not
// filled it in yet, but a half-filled row survives parsing so schema
// validation reports it instead of the form quietly dropping what was
// typed.
export function profileToRows(profile = {}) {
  return (Array.isArray(profile?.links) ? profile.links : [])
    .map((link) => ({ label: String(link?.label ?? ''), url: String(link?.url ?? '') }));
}

export function buildProfile(fields = {}) {
  const name = String(fields.name ?? '').trim();
  const bio = String(fields.bio ?? '').trim();
  const rows = Array.isArray(fields.linkRows) ? fields.linkRows : [];

  const links = rows
    .filter((row) => String(row?.label ?? '').trim() !== '' || String(row?.url ?? '').trim() !== '')
    .map((row) => ({ label: String(row?.label ?? '').trim(), url: String(row?.url ?? '').trim() }));

  const out = {};
  if (name) out.name = name;
  if (bio) out.bio = bio;
  if (links.length) out.links = links;
  // undefined (not {}) tells the API route the owner removed everything,
  // which deletes the profile block from the record rather than committing
  // a `profile: {}` that means nothing.
  return Object.keys(out).length ? out : undefined;
}

// ── Rows: one record per row, the way a DNS console shows them ──────────
//
// The registry stores records grouped by host and type (`records.A` is an
// array; `subdomains.blog.MX` is an array of { priority, value }). A table
// wants the opposite shape: one row per published record. These two convert
// between them, and they are the only place that mapping exists, so a round
// trip can be tested rather than trusted.
//
// `label` is '' for the claimed name itself and the subdomain label
// otherwise -- the same grammar schema.js validates, including the
// underscore forms (`_vercel`, `_vercel.recruitment`).
export const ROW_TYPES = ['A', 'AAAA', 'CNAME', 'TXT', 'MX'];

// URL is not a row: a redirect is served by the app, publishes no DNS at all,
// and has its own place in the UI. It must still survive a round trip, so
// recordsToRows carries it out of band rather than dropping it.
export function recordsToRows(record = {}) {
  const rows = [];
  let nextId = 0;
  const push = (label, type, value, priority) => {
    rows.push({ id: `r${nextId++}`, label, type, value, ...(priority === undefined ? {} : { priority }) });
  };

  const addHost = (label, records = {}) => {
    for (const type of ROW_TYPES) {
      if (!(type in records)) continue;
      const value = records[type];
      if (type === 'CNAME') push(label, type, String(value));
      else if (type === 'MX') {
        for (const entry of Array.isArray(value) ? value : []) {
          push(label, type, String(entry?.value ?? ''), Number(entry?.priority));
        }
      } else {
        for (const item of Array.isArray(value) ? value : []) push(label, type, String(item));
      }
    }
  };

  addHost('', record.records ?? {});
  for (const label of Object.keys(record.subdomains ?? {})) {
    addHost(label, record.subdomains[label] ?? {});
  }
  return rows;
}

// The inverse. Rows for the claimed name become `records`, everything else
// becomes `subdomains.<label>`, and same-type rows on one host regroup into
// the array the registry stores.
//
// `keep` carries record types the table does not model -- today only URL --
// straight back into `records`, so editing DNS on a name cannot silently
// delete its redirect.
export function rowsToRecords(rows = [], { keep = {} } = {}) {
  const records = {};
  const subdomains = {};

  for (const row of rows) {
    const label = String(row?.label ?? '').trim().toLowerCase();
    const type = row?.type;
    if (!ROW_TYPES.includes(type)) continue;

    const raw = row?.value;
    const value = type === 'TXT' ? String(raw ?? '').trim() : normalizeHostOrAddress(type, raw);
    // A row someone has added but not filled in is not a request to publish
    // an empty record.
    if (!value) continue;

    const host = label === '' ? records : (subdomains[label] ??= {});
    if (type === 'CNAME') host.CNAME = value;
    else if (type === 'MX') (host.MX ??= []).push({ priority: Number(row?.priority), value });
    else (host[type] ??= []).push(value);
  }

  for (const [type, value] of Object.entries(keep ?? {})) {
    if (!ROW_TYPES.includes(type) && value !== undefined) records[type] = value;
  }

  return { records, subdomains };
}

// Hostnames normalise (trailing dot, case); addresses only trim, because an
// IPv6 address is hex and case-insensitive but its text is compared as
// written, and lowercasing an IPv4 would be meaningless.
function normalizeHostOrAddress(type, raw) {
  return type === 'CNAME' || type === 'MX' ? normalizeHostname(raw) : String(raw ?? '').trim();
}

// Everything the table must refuse before the server does, per row, in the
// owner's words. The server still re-validates: this is the fast half, not
// the authority.
export function validateRow(row = {}, rows = []) {
  const type = row?.type;
  const label = String(row?.label ?? '').trim().toLowerCase();
  const value = String(row?.value ?? '').trim();

  if (!ROW_TYPES.includes(type)) return 'Pick a record type.';
  if (label !== '' && !SUBDOMAIN_LABEL_RE.test(label) && !SUBDOMAIN_NESTED_RE.test(label)) {
    return 'Use letters, numbers and hyphens for the label — no dots or spaces.';
  }
  if (!value) return 'Enter a value.';

  if (type === 'CNAME' && !HOSTNAME_RE.test(normalizeHostname(value))) {
    return 'This should be a hostname like you.github.io — no https:// and nothing after the address.';
  }
  if (type === 'A' && !IPV4_RE.test(value)) return 'Enter an IPv4 address, like 203.0.113.10.';
  if (type === 'AAAA' && !IPV6_LOOSE_RE.test(value)) return 'Enter an IPv6 address, like 2606:4700:3037::6815:7eb.';
  if (type === 'TXT' && value.length > 255) return 'A TXT value can be at most 255 characters.';
  if (type === 'MX') {
    const priority = Number(row?.priority);
    if (!Number.isInteger(priority) || priority < 0 || priority > 65535) {
      return 'Enter a priority between 0 and 65535. Lower numbers are tried first.';
    }
    if (!HOSTNAME_RE.test(normalizeHostname(value))) return 'Enter the mail server hostname, like mx.example.com.';
  }

  // DNS itself forbids this: a CNAME answers for every type on its host, so
  // nothing else may share that host.
  const sameHost = rows.filter((other) => other !== row && String(other?.label ?? '').trim().toLowerCase() === label);
  if (type === 'CNAME' && sameHost.length > 0) {
    return 'This name already has another record. A CNAME has to be the only record on its name.';
  }
  if (sameHost.some((other) => other?.type === 'CNAME')) {
    return 'This name already has a CNAME, which has to be the only record on its name.';
  }
  return null;
}

const HOSTNAME_RE = /^(?=.{1,253}$)([a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,}$/;
const SUBDOMAIN_LABEL_RE = /^_?[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$/;
const SUBDOMAIN_NESTED_RE = /^_[a-z0-9]([a-z0-9-]{0,61}[a-z0-9]?)?\.[a-z0-9]([a-z0-9-]{0,61}[a-z0-9]?)?$/;
const IPV4_RE = /^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){3}$/;
const IPV6_LOOSE_RE = /^[0-9a-fA-F:]*:[0-9a-fA-F:.]*$/;

// What the hosting-target field holds after a preset is picked. `autoFilled`
// is the value the form itself last inserted (null if none). A value the form
// inserted is the form's to replace -- by the new preset's prefill, or by
// nothing when that preset has none, so a GitHub Pages target never lingers
// under Vercel. Anything the user typed or loaded from their saved record is
// theirs and is kept.
export function presetTarget({ current = '', autoFilled = null, prefill = null }) {
  const untouched = !current.trim() || (autoFilled !== null && current === autoFilled);
  if (!untouched) return { value: current, autoFilled: null };
  return { value: prefill || '', autoFilled: prefill || null };
}
