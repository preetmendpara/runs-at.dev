...
export function classifyClaim(claim, probe) {
  ...
  if (!records.CNAME && !(records.A ?? []).length && !(records.AAAA ?? []).length) return 'card';
  if (!probe?.ok) return 'down';
  const host = `${claim.name}.runs-at.dev`;
...
// A nudge issue is titled `<name>.runs-at.dev ...`, so the name it concerns
...
export function issueName(title) {
  const match = /^([a-z0-9-]+)\.runs-at\.dev\b/.exec(String(title ?? ''));
...
export function diagnoseStuck(claim) {
  ...
  if (/vercel-dns/i.test(cname)) {
    if (challenges.length === 0) return 'vercel-no-challenge';
    // The zone mirror only publishes a challenge naming the claim's own
    // hostname (planZoneVerificationRecords), so one for the apex -- seven
    // owners pasted `vc-domain-verify=runs-at.dev,...` -- sits in the claim and
    // never reaches DNS. Telling them it was "published correctly" sent them
    // off to press Refresh on something that could never verify.
    const own = `vc-domain-verify=${claim?.name}.runs-at.dev,`;
    if (claim?.name && !challenges.some((v) => v.startsWith(own))) return 'vercel-wrong-challenge';
    return 'vercel-awaiting-verification';
  }
...
Vercel decides what to serve from the \`Host\` header, and \`${name}.runs-at.dev\` is not registered on your project, so nothing there matches it and your site is never served for this hostname. A CNAME to a \`.vercel.app\` address looks like it should work and never does.

**Fix:** add \`${name}.runs-at.dev\` as a domain on your Vercel project. Vercel will say the domain belongs to another team — it does, we own \`runs-at.dev\` — and offer you a \`vc-domain-verify=\` TXT challenge. Add that on [/manage](https://runs-at.dev/manage) as a subdomain record with label \`_vercel\` and type \`TXT\`, and replace the CNAME with the target Vercel shows you.`,
...
\`runs-at.dev\` belongs to us rather than to you, so Vercel wants proof you control this specific name before it will serve it.

**Fix:** in your Vercel project's domain settings, copy the \`vc-domain-verify=${name}.runs-at.dev,…\` TXT value it offers. Add it on [/manage](https://runs-at.dev/manage) as a subdomain record with label \`_vercel\`, type \`TXT\`. Save, give our DNS sync a minute, then press **Refresh** on Vercel.`,

  'vercel-wrong-challenge': (name, claim) => {
    const pasted = (claim.subdomains?._vercel?.TXT ?? []).filter((v) => typeof v === 'string' && v.startsWith('vc-domain-verify='));
    return `Your \`_vercel\` TXT challenge is for the wrong hostname, so we cannot publish it and Vercel can never verify \`${name}.runs-at.dev\`.

You have: \`${pasted.join('`, `')}\`
It needs to start with: \`vc-domain-verify=${name}.runs-at.dev,\`

A challenge naming \`runs-at.dev\` itself means the domain added on Vercel was \`runs-at.dev\` rather than your name. That domain belongs to us, and publishing its challenge would hand the whole registry to your account, so the sync refuses it.

**Fix:** in your Vercel project's domain settings, remove \`runs-at.dev\` if it is there and add \`${name}.runs-at.dev\`. Copy the \`vc-domain-verify=${name}.runs-at.dev,…\` value it offers into [/manage](https://runs-at.dev/manage) as the \`_vercel\` TXT in place of the current one, then press **Refresh** on Vercel.`;
  },

  'vercel-awaiting-verification': (name) => `Your CNAME and your \`_vercel\` TXT challenge are both published correctly, but \`${name}.runs-at.dev\` is still serving our profile card rather than your project — so Vercel has not finished verifying.

**Fix:** open your Vercel project's domain settings and press **Refresh** next to \`${name}.runs-at.dev\`. Verification usually needs that nudge once the DNS is in place.
...
If it still will not verify, say so here. A challenge value goes stale if the domain was removed and re-added on Vercel, in which case you need the new one.`,
  ...
  'platform-default-host': (name, claim) => `Your record points at \`${claim.records.CNAME}\`, your platform's default host, and nothing is answering for \`${name}.runs-at.dev\` there.
...
**Fix:** add \`${name}.runs-at.dev\` as a custom domain in your host's settings — GitHub Pages puts it under repo Settings → Pages → Custom domain; Netlify and Cloudflare Pages have the same under domain management.`,

  unknown: (name, claim) => `\`${name}.runs-at.dev\` resolves and holds a valid certificate, but it is serving our profile card rather than your site — which means your provider is not yet answering for this hostname.

Currently pointing at: \`${claim.records?.CNAME ?? (claim.records?.A ?? []).join(', ')}\`

**Fix:** whichever host you use, add \`${name}.runs-at.dev\` to it as a custom domain. DNS alone is not enough — the provider has to recognise the hostname before it serves anything for it.`,
};
...
---
Opened automatically by the daily health check, which noticed \`${name}.runs-at.dev\` is serving the registry's profile card instead of your site. It closes itself once your name starts serving. If this is wrong, or you meant to serve the card, just close it.`;
}
