...
export const SERVER_INFO = { name: 'runs-at.dev', version: '1.0.0' };

const TOOLS = [
  {
    name: 'check_name',
    description:
      'Check whether a runs-at.dev subdomain name is available to claim. Returns { available: true } or why it is not (taken, reserved, or an invalid_* grammar code). Free, no auth.',
    inputSchema: {
      type: 'object',
      properties: {
        name: {
          type: 'string',
          description: 'The name to check, without the .runs-at.dev suffix. 2-32 chars, lowercase letters, numbers, hyphens, no leading/trailing hyphen.',
        },
      },
      required: ['name'],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        name: {
          type: 'string',
          description: 'The claimed name, without the .runs-at.dev suffix.',
        },
      },
      required: ['name'],
    },
