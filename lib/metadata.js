...
// layout sets metadataBase to https://runs-at.dev, so a relative canonical
...
export function cardMetadata({ name, record, profile }) {
  const url = `https://${name}.runs-at.dev`;
  ...
  const title = profile?.name ? `${profile.name} (${name}.runs-at.dev)` : `${name}.runs-at.dev`;
  const description =
    bio || `${name}.runs-at.dev is a subdomain claimed by @${login} on the runs-at.dev registry.`;
  ...
  const banner = `https://runs-at.dev/banner/${name}`;

  return {
    // `absolute` bypasses the layout's '%s — runs-at.dev' template, which
    // would otherwise render 'shrey.runs-at.dev — runs-at.dev'.
