...
export function buildLlmsTxt() {
  const sections = [
    '# runs-at.dev\n\n> A free subdomain registry. Sign in with GitHub, claim a name like you.runs-at.dev, and point it at your own hosting.\n',

    block('When to use runs-at.dev', [
    ...
    block('What this is', [
      'runs-at.dev is a subdomain registry, not a top-level domain. A real TLD means an ICANN application (the 2026 round\'s evaluation fee alone is $227,000) plus operating a registry. This gets the same distinctive-address feeling for about $10 a year by giving away subdomains under one registered domain.',
      'Prior art: is-a.dev, js.org, eu.org.',
    ]),

    block('How to claim', [
      'Go to https://runs-at.dev, sign in with GitHub, and type a name. If it\'s available, claiming it writes a record to domains/<name>.json in the public registry and the name is live within seconds, no DNS to configure.',
      'Programmatically: GET /api/check?name=<name> answers availability; POST /api/claim { "name": "<name>" } claims it (requires a GitHub OAuth session cookie from https://runs-at.dev/api/auth/github).',
      'Eligibility: the GitHub account must be at least 30 days old with at least one public repository. 1 free domain included per GitHub account; additional domains may be granted by the administrator, and one account can own several.',
    ]),

    block('How to point a name at your own hosting', [
      'Two ways. From the site: sign in and open https://runs-at.dev/manage, pick a record type, fill it in, and save; it commits to domains/<name>.json and DNS follows within seconds. Or by pull request: edit domains/<name>.json, which CI validates against the schema, and DNS is updated on merge. Both enforce the same rules and both can set records and subdomains entries. Supported types: CNAME (a hostname, alone), A (array of IPv4), TXT (array of strings, up to 255 chars each), MX (array of { priority, value }, 1 to 5 entries, may coexist with A and TXT), and URL (an absolute http(s) redirect served by the app itself, alone, no DNS). An optional subdomains object adds one-level-deep records (e.g. _atproto or _discord) under the claimed name, same types except URL, up to 10 entries.',
      'Full record reference: https://runs-at.dev/docs/records. Copy-paste guides for hosts (Vercel, Netlify, GitHub Pages, Cloudflare Pages, Render, Railway, Firebase Hosting, Replit, Codeberg Pages), email forwarding, and social verification (Bluesky, Discord): https://runs-at.dev/docs/guides',
    ]),
    ...
    block('Static site hosting (deploy API)', [
      ...
      'Deploy tokens are minted self-serve at https://runs-at.dev/manage ("Deploy token" card): one-time display, 30-day TTL, scoped to publishing only the owner\'s own name (scope: sites:publish). Send as Authorization: Bearer <token>.',
    ]),

    block('For agents and machines', [
      'OpenAPI spec (every endpoint, typed): https://runs-at.dev/openapi.json',
      'MCP server (Streamable HTTP JSON-RPC, tools: check_name, get_record): https://runs-at.dev/.well-known/mcp',
      ...
      'API discovery: GET https://runs-at.dev/api returns the endpoint map as JSON.',
    ...
    block('The rules', [
      'Names are free and may be reclaimed if dormant, or immediately for impersonation, phishing, malware, or illegal content, no warning required. Full policy: https://runs-at.dev/policy',
      'Report abuse: abuse@runs-at.dev',
    ]),

    block('Links', [
      '- Blog / updates: https://runs-at.dev/blog',
      '- RSS feed: https://runs-at.dev/feed.xml',
      '- API index (endpoint map as JSON): https://runs-at.dev/api',
      '- Claim a name: https://runs-at.dev',
      '- Docs: https://runs-at.dev/docs',
      '- Quickstart: https://runs-at.dev/docs/quickstart',
      '- Record reference: https://runs-at.dev/docs/records',
      '- Guides: https://runs-at.dev/docs/guides',
      '- SEO on a name: https://runs-at.dev/docs/seo',
      '- OpenAPI spec: https://runs-at.dev/openapi.json',
      '- MCP endpoint: https://runs-at.dev/.well-known/mcp',
      '- Sitemap: https://runs-at.dev/sitemap.xml',
      '- About: https://runs-at.dev/about',
      '- FAQ: https://runs-at.dev/faq',
      '- Policy: https://runs-at.dev/policy',
      '- Contact: https://runs-at.dev/contact',
      '- Privacy: https://runs-at.dev/privacy',
      '- Registry source: https://github.com/preetmendpara/runs-at.dev',
      '- Operated by @preetmendpara: https://github.com/preetmendpara',
    ]),
  ];
