  ...
  return function take(key, now = Date.now()) {
    ...
    if (times.length >= max) {
      ...
      return {
        ...
        limit: max,
        remaining: 0,
        resetSec: Math.ceil(retryAfterMs / 1000),
      };
    }
    ...
    return {
      ok: true,
      limit: max,
      remaining: max - times.length,
      resetSec: Math.ceil(windowMs / 1000),
    };
  };
}

// IETF RateLimit headers (draft-ietf-httpapi-ratelimit-headers) built from a
// take() result, so every limited endpoint answers with the same shape and
// agents can self-throttle without knowing each route's private budget.
export function rateLimitHeaders(budget) {
  if (!budget?.limit) return {};
  return {
    'RateLimit-Limit': String(budget.limit),
    'RateLimit-Remaining': String(Math.max(budget.remaining ?? 0, 0)),
    'RateLimit-Reset': String(budget.resetSec ?? 0),
  };
}
