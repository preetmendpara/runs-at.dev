...
// A name only "should" be serving someone else's site when its records point
// there: CNAME, A or AAAA. TXT and MX publish real DNS without changing what a
// visitor sees, so the profile card answering is the correct outcome, not a
// fault -- lib/health.js classifies them that way too, and this must agree or
// the page contradicts the daily check.
function pointsAtOwnSite(records) {
  return Boolean(records?.CNAME || records?.A?.length || records?.AAAA?.length);
}

export function siteStatus(check, mode, records) {
  ...
  // Records that publish DNS without pointing the name anywhere (a TXT
  // verification code, an MX for email). `records` is undefined for callers
  // that only know the mode, which keeps their behaviour unchanged.
  if (status === 'card' && records && !pointsAtOwnSite(records)) {
    return {
      tone: 'live',
      label: 'Live',
      detail: 'Your DNS records are published. Visitors still see your profile card, because none of those records point the name at a site.',
    };
  }
...
const CARDS = [
  ...
  { id: 'advanced', title: 'DNS records', description: 'Add A, AAAA, CNAME, TXT and MX records yourself.' },
];
...
function inUseSummary(id, records = {}) {
  ...
  if (id === 'advanced') {
    const count = (records.A?.length ?? 0) + (records.AAAA?.length ?? 0)
      + (records.TXT?.length ?? 0) + (records.MX?.length ?? 0);
...
export function featureCards({ savedMode, records = {}, check = null, savedAt = null, now = Date.now() } = {}) {
  const live = siteStatus(check, savedMode, records);
...
// Per row: is this record actually in public DNS yet?
//
// /api/dns-check resolves the claimed name itself (and the `_vercel` label,
// which providers ask for by name). Rows on any other label are not looked
// up, and say so rather than guessing -- an unknown answer must never be
// drawn as a failure.
//
// `savedAt` gives the same publishing grace as the dashboard: a row saved a
// moment ago is mid-publish, not missing.
export function verifyRows(rows = [], check = null, { savedAt = null, now = Date.now() } = {}) {
  const publishing = savedAt !== null && now - savedAt < PUBLISHING_WINDOW_MS;
  const same = (a, b) => String(a).trim().toLowerCase().replace(/\.$/, '') === String(b).trim().toLowerCase().replace(/\.$/, '');

  return rows.map((row) => {
    if (!check) return { id: row.id, state: 'unknown', text: 'Checking…' };

    const label = String(row.label ?? '').trim().toLowerCase();
    let live;
    if (label === '') {
      if (row.type === 'CNAME') live = check.cname ?? [];
      else if (row.type === 'A') live = check.a ?? [];
      else if (row.type === 'AAAA') live = check.aaaa ?? [];
      else if (row.type === 'TXT') live = check.txt?.name ?? [];
      else if (row.type === 'MX') live = (check.mx ?? []).map((entry) => entry?.exchange ?? entry);
    } else {
      const answers = check.labels?.[label];
      if (answers) live = answers[row.type.toLowerCase()];
      // The claimed name's own _vercel TXT is resolved by the fixed lookups
      // whether or not the record declares it, so fall back to that rather
      // than reporting a challenge as unchecked.
      if (live === undefined && label === '_vercel' && row.type === 'TXT') {
        live = check.txt?.vercelLabel ?? [];
      }
      if (live !== undefined && row.type === 'MX') {
        live = live.map((entry) => entry?.exchange ?? entry);
      }
    }

    if (live === undefined) {
      return { id: row.id, state: 'unknown', text: 'Not checked here' };
    }
    if (live.some((value) => same(value, row.value))) {
      return { id: row.id, state: 'published', text: 'Published' };
    }
    if (publishing) return { id: row.id, state: 'publishing', text: 'Publishing…' };
    if (live.length === 0) return { id: row.id, state: 'missing', text: 'Not in DNS yet' };
    return { id: row.id, state: 'different', text: `DNS currently answers ${live[0]}` };
  });
}

// What stops working if this record goes. A record on the claimed name and a
// record on a label below it have different consequences, and saying "your
// name will go back to the profile card" about a label was wrong twice over:
// the label stops resolving, and the claimed name is not touched at all.
export function deleteConsequence(row = {}, name = '') {
  const label = String(row.label ?? '').trim().toLowerCase();
  const host = label ? `${label}.${name}.runs-at.dev` : `${name}.runs-at.dev`;
  const apex = `${name}.runs-at.dev`;

  if (!label) {
    if (row.type === 'CNAME') return 'Your name will stop serving that site and go back to the profile card.';
    if (row.type === 'MX') return `Email sent to ${apex} will stop being delivered.`;
    if (row.type === 'TXT') return 'A service that checks this code may stop treating the name as yours.';
    return 'Visitors will stop reaching that server.';
  }

  const unaffected = `Your main name, ${apex}, is not affected.`;
  if (row.type === 'CNAME') return `${host} will stop serving that site. ${unaffected}`;
  if (row.type === 'MX') return `Email sent to ${host} will stop being delivered. ${unaffected}`;
  if (row.type === 'TXT') return `A service that checks this code at ${host} may stop treating it as yours. ${unaffected}`;
  return `Visitors will stop reaching that server at ${host}. ${unaffected}`;
}
