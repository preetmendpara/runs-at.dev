...
// The branch registry reads and writes go to. Unset (the site) means the
// repository's default branch. The pr-slots workflow sets it to the pull
// request's base branch, so a reservation lands where the pull request merges
// and where validate counts from.
export const registryBranch = () => process.env.REGISTRY_BRANCH || undefined;
const refQuery = () => (registryBranch() ? `?ref=${encodeURIComponent(registryBranch())}` : '');
// Added to every contents-API write body.
export const onBranch = (body) => (registryBranch() ? { ...body, branch: registryBranch() } : body);
...
export async function getContentsMeta(path, { fetchImpl = fetch, token } = {}) {
  const res = await fetchImpl(`${API}/repos/${REPO}/contents/${path}${refQuery()}`, {
