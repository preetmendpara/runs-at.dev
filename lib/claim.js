...
// The one name every eligible account gets. An account's real allowance is
// this plus whatever the maintainer has granted (lib/entitlements.js), and
// callers pass it in. Lives here, not in entitlements.js, because this file
// also ships to the browser and that one reads the filesystem.
export const INCLUDED_DOMAINS = 1;
export const MAX_NAMES_PER_ACCOUNT = INCLUDED_DOMAINS;
...
// and how many it is allowed (included plus admin-granted slots).
export function withinNameLimit(login, name, ownedCount, allowed = MAX_NAMES_PER_ACCOUNT) {
  return ownedCount < allowed || isMaintainerProjectName(login, name);
}
...
export function evaluateClaim({ name, session, existing, now = new Date(), ownedCount = 0, allowed = MAX_NAMES_PER_ACCOUNT, country }) {
  ...
  if (!withinNameLimit(session.login, name, ownedCount, allowed)) {
