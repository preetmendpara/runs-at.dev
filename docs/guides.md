...
Two ways to get it there: paste it into [runs-at.dev/manage](https://runs-at.dev/manage),
...
No hosting, no DNS to configure. Visiting `you.runs-at.dev` sends the
...
`https://you.runs-at.dev` should 307-redirect to your target URL within a
few seconds. Full walkthrough: [`/docs/guides/url-redirect`](https://runs-at.dev/docs/guides/url-redirect).
...
Add `you.runs-at.dev` as a custom domain on the Vercel project (Project →
...
`_vercel.runs-at.dev` — the zone-level host Vercel actually reads the
...
[`/docs/guides/vercel`](https://runs-at.dev/docs/guides/vercel).
...
Add `you.runs-at.dev` as a custom domain in the Netlify site's settings so
it issues a certificate for it. Full walkthrough: [`/docs/guides/netlify`](https://runs-at.dev/docs/guides/netlify).
...
Pages repo itself containing `you.runs-at.dev`, GitHub's standard
custom-domain setup, independent of this registry. Full walkthrough:
[`/docs/guides/github-pages`](https://runs-at.dev/docs/guides/github-pages).
...
project's settings. Full walkthrough: [`/docs/guides/cloudflare-pages`](https://runs-at.dev/docs/guides/cloudflare-pages).
...
`you.runs-at.dev`; Render verifies the CNAME and issues a certificate on
its own. Full walkthrough: [`/docs/guides/render`](https://runs-at.dev/docs/guides/render).
...
walkthrough: [`/docs/guides/railway`](https://runs-at.dev/docs/guides/railway).
...
issuance can take a few hours. Full walkthrough: [`/docs/guides/firebase`](https://runs-at.dev/docs/guides/firebase).
...
walkthrough: [`/docs/guides/replit`](https://runs-at.dev/docs/guides/replit).
...
repo. Full walkthrough: [`/docs/guides/codeberg-pages`](https://runs-at.dev/docs/guides/codeberg-pages).
...
`you.runs-at.dev` already points at a site. Add `you.runs-at.dev` as a
domain at your forwarding provider and set up an alias before opening the
pull request. Full walkthrough: [`/docs/guides/email-forwarding`](https://runs-at.dev/docs/guides/email-forwarding).
...
record at `_atproto.you.runs-at.dev` containing your account's DID
...
my own domain → enter `you.runs-at.dev` → No DNS Panel → Verify DNS
Record. Full walkthrough: [`/docs/guides/bluesky-handle`](https://runs-at.dev/docs/guides/bluesky-handle).
...
`_discord.you.runs-at.dev`. The `dh=...` value is generated per domain by
...
[`/docs/guides/discord-verification`](https://runs-at.dev/docs/guides/discord-verification).
